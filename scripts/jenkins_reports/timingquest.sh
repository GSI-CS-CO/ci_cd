...
exp_log=nightly_build_exploder5.$(date +%Y-%m-%d).log
pex_log=nightly_build_pexarria5.$(date +%Y-%m-%d).log
vet_log=nightly_build_vetar2a.$(date +%Y-%m-%d).log
scu3_log=nightly_build_scu3.$(date +%Y-%m-%d).log
scu2_log=nightly_build_scu2.$(date +%Y-%m-%d).log
dm_log=nightly_build_ftm.$(date +%Y-%m-%d).log
vet_eebutis_log=nightly_build_vetar2a_eebutis.$(date +%Y-%m-%d).log
...
#Check if time quest analysis for vetar2a_eebutis was successful
if (grep -rqn $vet_eebutis_log -e "Running Quartus Prime TimeQuest Timing Analyzer"); then
  if (grep -rqn $vet_eebutis_log -e "Quartus Prime TimeQuest Timing Analyzer was successful"); then
    echo "Vetar2a_eebutis TimeQuest Timing Analyzer was successful"
  else
    echo "Vetar2a_eebutis TimeQuest Timing Analyzer was not successful"
  fi
else
  echo "Vetar2a_eebutis TimeQuest Timing Analyzer not started or log file not found"
fi
