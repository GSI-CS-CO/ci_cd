#!/bin/bash
#This script is written to check if there are any critical timing warnings during the nightly build of gateware for different formfactors
#Checking is done based on the status report generated during build process

#Variables
exp_job_path=/home/timing/jenkins_jobs/nightly_build_exploder5/syn/gsi_exploder5/exploder5_csco_tr/exploder5_csco_tr.sta.rpt
pex_job_path=/home/timing/jenkins_jobs/nightly_build_pexarria5/syn/gsi_pexarria5/control/pci_control.sta.rpt
vet_job_path=/home/timing/jenkins_jobs/nightly_build_vetar2a/syn/gsi_vetar2a/wr_core_demo/vetar2a.sta.rpt
scu3_job_path=/home/timing/jenkins_jobs/nightly_build_scu3/syn/gsi_scu/control3/scu_control.sta.rpt
scu2_job_path=/home/timing/jenkins_jobs/nightly_build_scu2/syn/gsi_scu/control2/scu_control.sta.rpt
dm_job_path=/home/timing/jenkins_jobs/nightly_build_datamaster/syn/gsi_pexarria5/ftm/ftm.sta.rpt
vet_eebutis_path=/home/timing/jenkins_jobs/nightly_build_vetar2a_ee_butis/syn/gsi_vetar2a/ee_butis/vetar2a.sta.rpt
...
#Check if gateware built for vetar2a_eebutis has any critical timing warnings
if [ -f $vet_eebutis_job_path ]; then
        if (grep -rqn $vet_eebutis_job_path -e "Critical Warning (332148): Timing requirements not met"); then
                echo "Vetar2a_eebutis has critical warning. Timing requirements not met"
        else
                echo "Vetar2a_eebutis has no timing critical warning"
        fi
else
        echo "Vetar2a_eebutis nightly build was not stable"
fi
