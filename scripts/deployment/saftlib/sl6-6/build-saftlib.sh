...
BASE_DIR=`pwd`
JOBS=32
DEPLOY_TARGET="/common/export/saftlib-dev"
...
for i in x86_64 i386; do
  ...
  ARCH=$i
  yumdownloader --destdir tmp dbus glib2.$ARCH libselinux.$ARCH libcap-ng.$ARCH audit-libs.$ARCH expat.$ARCH libsigc++20.$ARCH 
  rm -rf staging
  mkdir staging
  cd staging
  # Extract all rpms
  for i in ../tmp/*; do rpm2cpio "$i" | cpio -idmv; done
  # Copy stuff that is relevant for saftlib
  mkdir lib
  cd lib
  tar xJf ../../../../glibs.tar.xz
