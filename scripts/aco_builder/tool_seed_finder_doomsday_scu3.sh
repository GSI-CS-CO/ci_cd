...
# On successful completion of the synthesis, its timing report file is scanned
...
# Each build is evaluated and reported in log file, seed_finder.log.
...
function doBuildEvalLog {
  ...
  [[ ! -d "$BUILD_DIR" ]] && echo "$BUILD_DIR not exists. Exit!" && return

  cd $BUILD_DIR

  local MY_LOG_FILE="$(pwd)/seed_finder.log"

  [[ ! -d "$BEL_PRJ_DIR" ]] && echo "$BEL_PRJ_DIR not exists. Exit!" && return
  ...
  [[ $SYN_DIR = "" ]] &&
    echo "$CFG_TARGET is not found in Makefile. Exit!" >> $MY_LOG_FILE && return

  local SYN_PATH="$(pwd)/$SYN_DIR"

  echo "Synthesis location for $CFG_TARGET:  $SYN_PATH" >> $MY_LOG_FILE

  if [ -f $SYN_PATH/$QRTS_SETTINGS_FILE_EXT ]; then
    local QRTS_SETTINGS_FILE_PATH=$(ls $SYN_PATH/$QRTS_SETTINGS_FILE_EXT)
  else
    echo "Quartus settings file is not found. Exit!" >> $MY_LOG_FILE && return
  fi
  ...
  if [ $? -eq 0 ]; then
    ...
    # ==========================================================================
    # Analyse timing and log results
...
# ==============================================================================
# Build target images and evaluate report files
